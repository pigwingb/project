...
#define CONVERSION_TIME 10000 // conversion time in microseconds
...
float MS561101BA::getPressure(uint8_t OSR) {
  ...
  int64_t off  = ((uint32_t)_C1[1] <<16) + (((int64_t)dT * _C1[3]) >> 7);
  int64_t sens = ((uint32_t)_C1[0] <<15) + (((int64_t)dT * _C1[2]) >> 8);
...
int32_t MS561101BA::getDeltaTemp(uint8_t OSR) {
  ...
  if(rawTemp != 0) {
    return (int32_t)(rawTemp - ((uint32_t)_C1[4] << 8));
  }
  else {
    return NULL;
  }
